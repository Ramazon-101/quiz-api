...
class QuestionsListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        q_condection = Q()
        if sub:
            q_condection = Q(subject__iexact=sub)
        qs = qs.filter(q_condection)
        answers_questions = [j[0] for j in Answer.objects.filter(quiz__user=self.request.user).values_list('question')]
        questions = [j[0] for j in Questions.objects.filter(subject=sub).values_list('id') if
    ...
    def get(self, *args, **kwargs):
        ...
        for q in qs:
            data[str(q['quiz__created_at'])] = {
                q["quiz__user"]: {"true_count": q['quiz__true_count'], "answers_count":  5}}
